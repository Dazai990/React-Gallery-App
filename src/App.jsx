...
const App = () => {
  ...
  let printImageData = (
    <h3 className="text-gray-400 font-bold absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
      Loading...
    </h3>
  );
    ...
    <div className="min-h-screen text-black bg-gray-200">
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white border-t-2 border-black/20 flex justify-center gap-6 items-center p-5">
        <button
          onClick={handlePrev}
          disabled={index === 1}
          className={`bg-gray-200 font-mono transition ease-in-out text-sm active:scale-95 cursor-pointer text-black rounded-xl py-2 px-4 font-semibold
            ${
              index === 1
          ...
          `}
        >
          <StepBack className="transition ease-in-out" />
        </button>
        ...
        >
          <StepForward className="transition ease-in-out" />
        </button>
      </div>
    </div>
  );
